...
/**
  * @brief TIM2 Initialization Function (1MHz free-running counter)
  * @param None
  * @retval None
  */
static void MX_TIM2_Init(void)
{
  ...
  htim2.Instance = TIM2;
  // TIM2位于APB1域：当APB1分频器 != 1 时，定时器时钟 = PCLK1 * 2
  uint32_t pclk1 = HAL_RCC_GetPCLK1Freq();
  uint32_t apb1_presc = (RCC->CFGR & RCC_CFGR_PPRE1) >> RCC_CFGR_PPRE1_Pos;
  uint32_t tim2_clk = (apb1_presc >= 4u) ? (pclk1 * 2u) : pclk1; // 0xxx=DIV1; 100=DIV2及以上
  htim2.Init.Prescaler = (tim2_clk / 1000000u) - 1u; // 1MHz
...
static void MX_GPIO_Init(void)
{
  ...
  // 传感器电源控制引脚初始化为断电状态
  // 低电平=断电，高电平=上电
  HAL_GPIO_WritePin(GPIOB, GAS_PWR_CTRL_Pin|RADAR_PWR_CTRL_Pin, GPIO_PIN_RESET);
