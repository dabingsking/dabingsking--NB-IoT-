...
#define IMU_USART1_DEBUG	0

static float accSensitivity   = 0.244f;   //加速度的最小分辨率 mg/LSB
static float gyroSensitivity  = 32.8f;    //陀螺仪的最小分辨率

icm42688RawData_t *test_GyroData;
icm42688RawData_t *test_AccData;

uint8_t cover_tilt_value = 0;
...
#define ICM_SPI_CS_LOW()    GPIO_ResetBits(ICM_PORT_SPIX_CS, ICM_PIN_SPIX_CS)
#define ICM_SPI_CS_HIGH()   GPIO_SetBits(ICM_PORT_SPIX_CS, ICM_PIN_SPIX_CS)
...
//        *pBuffer = hal_Spi2_ReadWriteByte(*pBuffer);
			  *pBuffer = SPI1_ReadWriteByte(*pBuffer);
        pBuffer++;
    }
#endif
...
int8_t bsp_Icm42688RegCfg(void)
{
    ...
    test_AccData = (icm42688RawData_t *)mymalloc(sizeof(icm42688RawData_t));
    test_GyroData = (icm42688RawData_t *)mymalloc(sizeof(icm42688RawData_t));
...
float max_absolute(float a, float b) 
{
    /* 版本1：使用标准库函数（推荐）*/
...
{
	bsp_IcmGetRawData(test_AccData, test_GyroData);delay_ms(1);
	...
	#if(IMU_USART1_DEBUG==1)
	printf("pitch:%.1f roll:%.1f yaw:%.1f\r\n",Q_ANGLE_X, Q_ANGLE_Y, Q_ANGLE_Z);
	#endif
	
	cover_tilt_value = (uint8_t)max_absolute(Q_ANGLE_X, Q_ANGLE_Y);
}
